...
def DefiniEstilo(ttk):
    ...
    style = ttk.Style()
    # Você pode escolher outro tema base, como 'default' ou 'alt'
    style.theme_use('clam')
    # Estilo para Checkbutton
    style.configure("TCheckbutton", background=cor_fundo,
                    foreground=cor_checkbox, font=fonte_texto)
    style.map("TCheckbutton", foreground=[
              ("active", cor_checkbox)], background=[("active", cor_ativo)])

    # Estilo para Label (texto)
    style.configure("TLabel", background=cor_fundo,
                    foreground=cor_texto, font=fonte_texto)
    ...
    style.configure("Custom.TFrame", background=cor_fundo,
                    foreground=cor_fundo)

    style.configure("CustomLabel.TLabel", foreground=cor_texto,
                    background=cor_fundo, font=("Helvetica", "11"))

    style.configure("Horizontal.TProgressbar",
                    background=cor_barra, troughcolor=cor_fundo)
